...
"""A live voice agent that rolls dice, checks primes, and can see.

No transport code: `adk web` runs it, and both entry points in this directory
put this same `root_agent` behind a LiveKit room without changing this file.
"""
...
root_agent = Agent(
    ...
    instruction="""
      ...
      If the user has a camera on you can see them, so answer questions about what is in view.
      If they hold up a real die, read the number off it and you may use that number with check_prime, since you did not roll it.
    """,
    tools=[
        roll_die,
        check_prime,
        # Offers nothing when there is no call, so `adk web` still works.
        LiveKitToolset(),
    ],
